...
HOST = "arboghast"
ROOT = Path("/home/amodo/exact-b-k21-k22-context-refinement-forward-20260909")
INPUTS = (
    (21, 353094,
     "f404c9f1e00bf40b24b1996ba31ee87c26cadfa70c2ca6140b04891685cb7392",
     Path("/home/amodo/k21_upper353094_context_refinement_forward_input.word")),
    (22, 706188,
     "aef0f78939dec65f60ffffe3c1ce77999fdacdd3034964684884b7e5352d5a59",
     Path("/home/amodo/k22_upper706188_context_refinement_forward_input.word")),
)
...
def scan_word(k, expected_length, expected_sha, source):
    ...
    (ROOT / f"k{k}_upper{expected_length}.word").write_bytes(raw)
    base.update(
...
def main():
    ...
    try:
        ...
        raise
    report = dict(
        status="PASS_FULL_CUBES" if all(r.get("upper_bound_verified")
        ...
        scope=("Only the two SHA-pinned supplied upper-bound words and their "
               "all-rank endpoint lower bounds. This is no proof of optimality, "
               "no reconstruction of the user's constructor, and no replay of "
               "an unprovided optimization or context schedule."),
    )
    write_json(ROOT / "k21_k22_context_refinement_forward_complete_certificate.json", report)
    print(json.dumps(dict(status=report["status"],
                          report=str(ROOT / "k21_k22_context_refinement_forward_complete_certificate.json"),
