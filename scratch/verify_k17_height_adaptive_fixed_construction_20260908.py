#!/usr/bin/env python3
"""One canonical height-adaptive construction, no cut search. h100 only."""
import hashlib
import json
import math
import resource
...
signal.alarm(150)
OUT=Path('/home/amodo/exact-b-k17-height-adaptive-20260908')
OUT.mkdir(parents=True, exist_ok=True)
R=8; N=17; FULL=(1<<N)-1

# Same original map and canonical cut convention as the earlier inventory.
...
assert len(set(fm.values()))==len(states)==24310
...
for cid,cycle in enumerate(cycles):
    ...
    for i in range(length):
        ...
        assert mask
        letters.append(mask)
    # Literal periodic dilation must recover the central owner exactly.
    for i in range(length):
        value=0
    ...
    records.append(dict(cycle=cid,first_lower_owner=cycle[0],length=length,
        height=h,min_positive_run=min(runs),lower_owners=cycle,source_period=letters,
...
assert sum(rec['height'] for rec in records)==519
assert len(theorem)==25202 and len(trimmed)==24829
...
def certify(word,label,witnesses=None):
    ...
    return dict(word_file=label+'.word',length=len(word),sha256=hashlib.sha256(raw).hexdigest(),
...
theorem_report=certify(theorem,'k17_height_theorem25202')
trim_witnesses=target_witnesses(trimmed)
trim_report=certify(trimmed,'k17_height_trimmed24829',trim_witnesses)
holes=trim_report['missing_targets']
repaired=trimmed+holes
repair_report=certify(repaired,'k17_height_repaired'+str(len(repaired)))
assert repair_report['missing_count']==0

thresholds=[]
for r,q,expected in [(283,10,1),(284,10,-1),(31115,100,1),(31116,100,-1)]:
    width=math.comb(2*r+1,r)
    numerator=(1<<(2*r+2))-3*width
    denominator=(2*r+1)*width
    difference=q*numerator-denominator
    sign=(difference>0)-(difference<0)
    assert sign==expected
    scale=10**20
    lower=numerator*scale//denominator
    thresholds.append(dict(r=r,comparison_denominator=q,sign_of_q_numerator_minus_denominator=sign,
        exact_integer_comparison_pass=True,lower_scaled_1e20=lower,upper_scaled_1e20=lower+1,
        decimal_interval_denominator=scale))

report=dict(status='PASS' if not theorem_report['missing_count'] else 'THEOREM_WORD_FAILED',
    dimension=17,canonical_convention='f(A)=complement(A) minus unique unmatched zero; g=f^2; each lower cycle starts at its least integer mask, cycles ordered by that mask; future-(h+1)-intersection source, no cut or order search',
    component_count=len(cycles),height_sum=sum(rec['height'] for rec in records),
    ...
    all_periodic_owner_reconstructions_exact=True,
    theorem=theorem_report,trimmed=trim_report,repaired=repair_report,
    claimed_140_holes_match=(len(holes)==140),threshold_checks=thresholds,
...
print(json.dumps({k:v for k,v in report.items() if k not in ('theorem','trimmed','repaired')},indent=2))
for label,rec in [('theorem',theorem_report),('trimmed',trim_report),('repaired',repair_report)]:
    print(label,json.dumps({k:v for k,v in rec.items() if k!='missing_targets'},sort_keys=True))
print(report['status'], 'one fixed canonical construction and exact threshold checks; no search')
