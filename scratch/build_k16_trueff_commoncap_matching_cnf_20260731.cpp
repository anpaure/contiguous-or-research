...
string file_sha256(const string& path){
  const string command="sha256sum -- "+shell_quote(path);
  FILE* pipe=popen(command.c_str(),"r");
  if(!pipe)throw std::runtime_error("cannot launch sha256sum");
  array<char,256> buffer{};
  string line;
  if(fgets(buffer.data(),static_cast<int>(buffer.size()),pipe))line=buffer.data();
  const int status=pclose(pipe);
  if(status!=0||line.size()<65)throw std::runtime_error("sha256sum failed");
  const string digest=line.substr(0,64);
  if(!std::all_of(digest.begin(),digest.end(),[](unsigned char ch){return std::isxdigit(ch);}))
    throw std::runtime_error("malformed sha256sum output");
  return digest;
}
...
vector<uint16_t> load(const string& path){
  ...
  const string raw((std::istreambuf_iterator<char>(in)),std::istreambuf_iterator<char>());
  if(in.bad())throw std::runtime_error("target read failure");
  std::istringstream parsed(raw);vector<uint16_t> out;uint64_t value;
  while(parsed>>value){
    if(value>FULL)throw std::runtime_error("target value outside uint16 range");
    out.push_back(static_cast<uint16_t>(value));
  }
  parsed.clear();parsed>>std::ws;
  if(!parsed.eof())throw std::runtime_error("trailing malformed target input");
...
int main(int argc,char**argv){
  ...
  const string target_sha=file_sha256(target_path);
  if(target_sha!=EXPECTED_SHA)throw std::runtime_error("target SHA-256 mismatch");
  ...
  if(reserved!=12781)throw std::runtime_error("reserved singleton cell regression");
  ...
  for(int i=0;i<W;++i){
    for(int bit=0;bit<K;++bit)if(targets[i]&(1u<<bit)){
      ...
      for(int len=1;len<=H;++len)for(int s=std::max(0,hi-len+1);s<=lo;++s){
        if(s>=L)continue;
        int cell=lookup[s][len];
        if(cell>=0)mandatory[cell]|=(1u<<bit);
      }
    }
  }
  ...
  if(avec.size()!=70759||a0!=347678)throw std::runtime_error("letter-bit variable regression");
  ...
  if(variables!=1055230||expected!=4513893)
    throw std::runtime_error("DIMACS size regression");

  Writer cnf(prefix+".cnf");cnf.out<<"p cnf "<<variables<<' '<<expected<<"\n";
  // Exactly one cell for every remaining lower target.
  for(int s=1;s<(1<<K);++s)if(pc(static_cast<uint16_t>(s))<R&&s!=PIN){
    for(int v:by_target[s])cnf.lit(v);
    cnf.end();
    emit_amo(cnf,by_target[s],target_aux[s]);
  }
  ...
  for(int i=0;i<W;++i)for(int bit=0;bit<K;++bit)if(targets[i]&(1u<<bit)){
    for(int p=starts[i];p<=deadlines[i];++p)if(avar[p][bit])cnf.lit(avar[p][bit]);
    cnf.end();
  }
