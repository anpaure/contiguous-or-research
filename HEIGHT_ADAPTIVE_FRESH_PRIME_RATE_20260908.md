...
2026-09-08. This is the supplied full proof, retained after complete root
reading and [independent internal proof review](scratch/FRESH_PRIME_REVERSE_PROCESS_AND_EXPLICIT_ONSET_AUDIT_20260908.md).
The arithmetic, conditional probability, fixed-size transfer and explicit
onset pass that review on the stated finite PBBS inputs. Internal AI
review is not external or formal certification. The user's reported test
package was not supplied or rerun here; no reported test count is a premise.
The separate [24,658-letter certificate](K17_UPPER24658_VERIFIED_20260908.md)
was independently executed on its actual supplied word.

Comparison: this rate is weaker asymptotically than the already recorded
logarithmic-gcd scale exp[-(k(log k)^2)^(1/5)/128]. Its new benefit is the
explicit onset 2^2048+1, substantially below that route's earlier recorded
onset. The later depth-product note improves the logarithmic-gcd coefficient.

## 1. Main result and scope

Let W(k)=binomial(k,floor(k/2)). Retain the height-adaptive word construction
from HEIGHT_ADAPTIVE_PBBS_CONSTRUCTION_20260908.md. For odd n its length is
...
These inputs are consolidated in HEIGHT_ADAPTIVE_MULTILEVEL_PROFILE_BOUND_20260908.md,
HEIGHT_ADAPTIVE_LOG_GCD_RATE_20260908.md, and the finite
