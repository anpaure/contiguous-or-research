...
def main():
    ...
    k = 18
    full = (1 << k) - 1
    assert len(word) == 48623, ('incorrect length', len(word))
    ...
    for target in range(1, full + 1):
        ...
        rechecked += 1
    width = comb(18, 9)
    lower_targets = sum(comb(18, rank) for rank in range(1, 9))
    delay = 0
    while delay * width + delay * (delay + 1) // 2 < lower_targets:
        delay += 1
    assert (width, lower_targets, delay, width + delay) == (48620, 106761, 3, 48623)
