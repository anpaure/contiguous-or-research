...
The supplied25374 word passed this checker on2026-09-08. It enumerates
all suffix unions and independently replays every saved interval witness.
"""
import argparse
import hashlib
import json
...
def main():
    ...
    full = (1 << 17) - 1
    assert len(word) == 25374, ('incorrect length', len(word))
    ...
    (args.output_directory / 'literal25374_verification.json').write_text(
        json.dumps(report, indent=2) + '\n')
    (args.output_directory / 'literal25374_target_witnesses.json').write_text(
