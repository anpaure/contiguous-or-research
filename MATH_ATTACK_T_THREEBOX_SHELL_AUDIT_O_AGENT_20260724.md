...
1. **Literal triangular-shell word: valid after a boundary repair in the
   ...
   for the width, and the identity relating length to width are exact.  Only
   the first displayed witness recipe fails, and only in the subcase
   \(t=1,x=1\), because the initial \(A_1\) was deleted.  The theorem remains
...
3. **Canonical-ring obstruction: certified for the explicit outward
   enumeration defined here.**  The raw report omitted the required
   enumeration and congestion proof, so its word “canonical” cannot be
   matched uniquely to an intended cut.  Section 5 supplies one natural
   precise interpretation.  For that order it proves the reported inequality
   \[
...
targets.  Let \([u,v]\) be an internal positive run, choose any
\(i\in[u,v]\), and let \(p\in I_i\) be an occurrence of that increment.
Since \(\ell_i>\ell_{u-1}\), an occurrence with
\(p\le r_{u-1}\) would lie in the negative interval \(I_{u-1}\), which is
impossible.  Hence \(p>r_{u-1}\).  Similarly,
\(r_i<r_{v+1}\), so an occurrence with \(p\ge\ell_{v+1}\) would lie in the
negative interval \(I_{v+1}\).  Hence \(p<\ell_{v+1}\).  Thus \(p\) lies
strictly between the two neighboring negative intervals, and consequently
\[
...
### 3.1 Generic cases and their exact scopes

Let \((x,y,z)\ne(0,0,0)\), and let
\(t=\max(x,y,z)\).  Use the following priority order.

**Case 1: \(y=t\).**  For \(t\ge2\), the interval
\[
...
This includes all ties involving \(y=t\).  The same recipe also works when
\(t=1,x=0\), but its \(t=1,x=1\) subcase is treated in Section 3.2.
...
The endpoint cases \(x=0\) and \(y=0\) are included.  This recipe remains
valid at \(t=1\): it gives \([C_1]\) for \(x=0\) and
\([C_1,A_1]\) for \(x=1\).

**Case 3: \(x=t\), \(y<t\), and \(z<t\), with \(t\ge2\).**  Begin in the
third side of \(\mathcal C_{t-1}\) at
\[
(t-1-z,0,z),
\]
...
Case 2 remains valid at \(t=1\), whereas Case 3 has \(t\ge2\) by
definition.  The remaining max-one cases may be checked uniformly as
follows.
...
\]
The positive-radius-\(r\) ring has \(6r\) vertices.  After restriction to
that ring, each of
\[
...
vertices.  Globally the same threshold can also meet outer rings, but this
does not affect the argument below.  These three arcs are disjoint.  A
linear cut of the cyclic ring
has two boundary vertices, so at least one of the three arcs contains
neither boundary vertex; its two immediate zero neighbors are then inside
the same ring block, so it remains an internal positive run in the full
order.  This
...
The raw report did not define its canonical cut.  The following explicit
outward enumeration supplies one natural precise interpretation and proves
the claimed constant for that stipulated order.  It does not establish
that this was the raw author's uniquely intended convention.
...
\tag{5.4}
\]
They are respectively the restrictions to the radius-\(r\) block of the
supports of \(y\ge r\) and \(z\ge r\), have \(r+1\) vertices, and hence
have run parameter \(r\).  Their immediate zero neighbors make them
positive runs in the full incidence word even though the same thresholds
can also be positive on outer rings.
...
- all these ranges stay inside their own ring except that the \(Z_1\)
  range may end at \(c_2\), the first index of ring \(2\);
- that exceptional \(Z_1\) range is still disjoint from every later
  run's \(\alpha\)-range, because \(B_{2,0}\cup B_{2,1}\) is assigned
  backward and has no \(\alpha\)-term, while the next such range starts
  only at \(Y_2\);
- the only overlap of distinct listed charges is therefore the two
  ring-one forward charges with the \(Y_1\) charges.
...
\tag{5.9}
\]
Therefore the numerical constant claimed in the raw report is valid for the
explicit outward enumeration (5.1)--(5.3), and is nonsharp there.  Without
a definition of “canonical,” the raw statement itself remains
under-specified rather than universally certified.
...
The index distance between consecutive selected row endpoints is in fact at
most \(4a\).  The prefix and suffix gaps from the two order boundaries to
\(e_0\) and \(e_{2a}\) are at most \(a\), since each lies within its own
row.  Partition the order into contiguous nearest-endpoint cells, each
containing its selected endpoint.  Every cell has size at most
\(H=4a+2\), and every offset increment is crossed by at most \(H\)
assignment paths.  Monotonicity gives
\[
d_i\le
\begin{cases}
...
Let \(p,q,r\) be positive integers, let \(R\to\infty\), and suppose
\[
\delta R\le p,q,r\le CR
\]
with fixed \(\delta,C>0\).  Let \(h\) be a maximum central rank of
\([0,p]\times[0,q]\times[0,r]\), put
\[
M=|\{(x,y,z):x+y+z=h\}|=w(p,q,r),
\]
and consider a word of length \(N=M+D\) whose selected maximum-rank
witnesses induce the stated raster.  In the fixed-\(x\) raster, the
\(y\)-range of row \(x\) is
\[
...
After omitting a possible endpoint with \(U_x=0\), for which
\(y\ge U_x\) is not a positive chain increment, all remaining endpoint
threshold components have uniformly bounded size.  The omitted endpoint
may instead be treated with the corresponding \(x\)-threshold, or simply
charged by \(D\).  Only \(O(1)\) sampled endpoints, lying within a constant
number of rows of the two global order boundaries, can have boundary
components; they too contribute only \(O(D)\).
...
endpoint.  Let the physical word be \(P_1,\ldots,P_n\), and assume that it
represents each \(T_i\) by a nonempty integer interval
\[
J_i=[a_i,b_i],
\]
with both endpoint sequences nondecreasing and with the coordinatewise
identity
\[
\bigvee_{j=a_i}^{b_i}P_j=T_i.
\]
...
\tag{7.3}
\]
for \(R\ge1\).  The equality is an occurrence-representation statement at
\(R=1\); \(W_1=B_1,C_1,A_1\) is not a universal cube word.  Its missing
\((1,1,0)\) target is supplied in \(W_R\) by \(\mathcal C_2\), so the
universal construction in Section 3 requires \(R\ge2\).
...
- For \(R\ge2\), \(W_R\) is an unconditional literal word of length
  \((2-o(1))w_R\).
- Every universal word whose selected middle witnesses induce complete
  ...
  \(2/21\).
- The explicit outward ring enumeration of Section 5 has certified leading
  constant \(1/4\), stronger than the report's under-specified \(1/5\).
...
one without additional hypotheses.  Suppose a word has length \(M+D\),
where its selected maximum-rank family has \(M=\Theta(R^2)\), its rank cap
is \(h=O(R)\), and it must also cover \(\Omega(R^3)\) distinct nonzero
targets below that rank.  Suppose the induced order of that family has:

1. \(\Theta(R)\) sampled targets whose consecutive index gaps, as well as
   the prefix and suffix gaps to the order boundaries, are \(O(R)\);
...
4. a contiguous-cell assignment in which every cell contains its sample
   and has size \(O(R)\), or more generally an assignment whose paths cross
   every offset increment only \(O(R)\) times.

Then
\[
\sum_i d_i=O(R^2+RD),
\]
and the rank-capped-start inequality with the stated lower volume and rank
cap forces
