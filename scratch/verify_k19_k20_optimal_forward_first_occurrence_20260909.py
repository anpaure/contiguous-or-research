...
HOST = "arboghast"
ROOT = Path("/home/amodo/exact-b-k19-k20-optimal-forward-20260909")
INPUTS = (
    (19, 92381, Path("/home/amodo/k19_optimal92381_forward_input.word")),
    (20, 184759, Path("/home/amodo/k20_optimal184759_forward_input.word")),
)
CAPS = dict(cpu_seconds=60, wall_seconds=90,
            address_space_bytes=1024**3, maximum_file_bytes=256*1024**2)
...
def scan_word(k, expected_length, source):
    ...
    # with ZERO-based inclusive ordinary endpoints. It uses about8 MiB at k20.
    witness_files = {}
...
def main():
    ...
    write_json(ROOT / "run_started.json", dict(status="RUNNING_NOT_CERTIFIED",
               ...
               resource_caps=CAPS))
    try:
        results = [scan_word(k, length, path) for k, length, path in INPUTS]
    except MemoryError:
        ...
        raise
    report = dict(
        status="PASS" if all(r.get("optimality_certified") for r in results) else "FAIL",
        ...
        scope="Only the two fixed supplied literal words and their all-rank endpoint lower bounds.",
    )
    write_json(ROOT / "k19_k20_forward_complete_certificate.json", report)
    print(json.dumps(dict(status=report["status"],
                          report=str(ROOT / "k19_k20_forward_complete_certificate.json"),
