# Height-adaptive depth product: coefficient 3/5 with an explicit onset

2026-09-08. Consolidated after complete root reading and independent
local-analytic and probability/parity reviews. The full downstream proof
below is retained from the [independent finite audit](scratch/PBBS_DEPTH_PRODUCT_FINITE_PROBABILITY_PARITY_AND_ONSET_AUDIT_20260908.md).
The [sharp residue/NB proof and exact integral certificate](scratch/PBBS_SHARP_UNIMODAL_NB_AND_DEPTH_INTEGRAL_INDEPENDENT_AUDIT_20260908.md)
are its complete upstream proof. Their bounded rational calculation was
independently executed on h100. No user-reported checker suite or search
was assumed to have run here. These are internally reviewed deductions
on the explicitly retained finite PBBS inputs, not external or formal
certification of those inputs.

This is an intermediate relative-error bound, later sharpened by the
[harmonic-period estimate](HEIGHT_ADAPTIVE_HARMONIC_PERIOD_RATE_20260908.md).
It improves
the prior 1/128 coefficient to3/5, a factor384/5=76.8, and supplies an
explicit sufficient dimension where the submission left its onset open.
The fresh-prime and stopped-LCM arguments are valid alternative routes,
but their eventual exponents do not improve this depth-product result. The finite
[24,658-letter word](K17_UPPER24658_VERIFIED_20260908.md) is a separate
certificate and is not a consequence of this asymptotic inequality.
...
[the sharp residue/NB/integral audit](scratch/PBBS_SHARP_UNIMODAL_NB_AND_DEPTH_INTEGRAL_INDEPENDENT_AUDIT_20260908.md), which was read for this audit. It proves the following uniformly on the domain in §1.
...
[depth_product_rational_certificate_20260908.json](scratch/depth_product_rational_certificate_20260908.json)
proves
...
[the earlier fixed-size profile audit](scratch/PBBS_REVERSE_PROFILE_FIXED_SIZE_TRANSFER_EXPLICIT_DOMAIN_AUDIT_20260908.md).
...
[the whole-row and tower audit](scratch/PBBS_LOG_GCD_WHOLE_ROW_LAW_AND_SUCCESSIVE_CONDITIONING_AUDIT_20260908.md), §§1–2 and 5. The inverse fixed-size conditioning probability is bounded by
