#!/usr/bin/env python3
"""Build the complete owner-only reflected-pair atlas inside a fixed free set.

Every row of the allowed set is used once as an anchor target.  The anchored
generator is complete for masks containing that row, so deduplicating the
resulting catalogues gives the complete mask atlas inside the allowed set.
"""
...
def main() -> None:
    ...
    assert args.jobs == 2 and args.threads_per_job == 40
    ...
    def generate(target: int) -> dict[str, object]:
        ...
        assert report["target_row"] == target
    ...
    for entry in entries:
        ...
        for record in read_catalog(Path(str(entry["catalog"]))):
            ...
            total_records += 1
            previous_status = status_by_mask.setdefault(rows, record["status"])
            assert previous_status == record["status"]
    ...
    ordered_records = [retained[rows] for rows in sorted(retained)]
    new_records = [record for record in ordered_records if record["status"] == "new"]
    frozen_records = [
        record for record in ordered_records if record["status"] == "frozen"
    ]
    assert len(ordered_records) == len(new_records) + len(frozen_records)
    write_catalog(all_catalog, ordered_records)
    write_catalog(new_catalog, new_records)

    manifest: dict[str, object] = {
        "status": "PASS",
        "scope": "complete owner-only reflection-pair masks contained in fixed F208",
        "instance": str(instance),
        ...
        "unique_pair_masks": len(ordered_records),
        ...
        "completeness": (
            "all 208 allowed rows were anchored; every retained 10-row mask "
            "occurred in exactly its ten constituent target catalogues"
        ),
    }
    ...
    print(json.dumps({
        ...
        for key in (
            "status",
            "allowed_row_count",
            "per_target_record_sum",
            "tenfold_membership_histogram",
            "unique_pair_masks",
            "frozen_pair_masks",
